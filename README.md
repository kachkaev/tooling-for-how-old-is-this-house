...
Если у вас возникает проблема с одним из источников, вы можете продолжать выполнение любых команд, которые не упоминают этот источник.
...
Такая защита позволяет проводить любые эксперименты без страха что-то испортить.
...
1.  **Скачать данные с Росреестра**
    ...
    ОКС (объекты капитального строительства) и земельные участки с геопривязкой:
...
1.  **Создать локальную картотеку геокодов**
    ...
    Собрав локальную картотеку геокодов, мы уменьшим количество объектов без координат.
...
1.  **Заполнить пробелы в картотеке геокодов при помощи Яндекса**

    Полнота данных ОСМ на выбранной территории существенно влияет на количество пробелов в локальной картотеке геокодов.
    ...
    После получения геокодов из Яндекса следует добавить их в локальную картотеку:
...
1.  **Подготовить слои для склейки**
    ...
    Некоторые из них обращаются к локальной картотеке геокодов, созданной ранее.
...
> 🚧 Настройка размеров и других параметров постера не реализована.
...
Как следствие, ошибки в источниках неизбежно попадают в итоговый набор данных.
...
Название файла может быть любым (например, `bridges.geojson`), а число таких файлов не ограничено.
...
> 🚧 Значение поля `dataToIgnore` пока что игнорируется.
> Тем не менее, его уже рекомендуется использовать в качестве задела на будущее.
