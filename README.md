...
Чтобы создать новую карту возраста зданий, вам понадобятся:
...
1.  Расширьте список кадастровых номеров, если это необходимо.
    ...
    ***
    ...
    Вероятность такой ситуации крайне мала; вы узнаете о ней только после склейки и визуализации всех данных.
    ...
    После обновления файла `territory-config.yml` надо перезапустить команду `4-generateInitialObjectInfoPages` и все последующие, которые связаны с Росреестром и склейкой слоёв.
...
Сопоставление адресов и географических координат из разных источников существенно улучшает качество склейки данных.
...
Единица склейки данных — стандартизированный файл (слой).
...
>
> Цвета зданий можно попробовать настроить в файле `/path/to/tooling/src/shared/completionDates.ts`.
...
Чтобы исключить данные по снесённому зданию из склейки, мы создаём файл `/path/to/data/territories/TERRITORY_NAME/sources/manual/patches.geojson` (`"layerRole": "patch"`).
...
Чтобы исправить другие случаи неправильной склейки, следует добавить больше объектов `Feature` в этот же файл.
...
После обновления файлов `/path/to/data/territories/TERRITORY_NAME/sources/manual/*.geojson`, следует повторить команды в разделе «Склейка слоёв».
...
Она создаст копию результата в подходящи для сайта формате.
